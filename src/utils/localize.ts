/**
 * Pure-function counterpart of the useLocalizedChar hook.
 * Used in non-React contexts (utils, analytics, etc.) to avoid duplicating
 * the lang-detection ternary chain across the codebase.
 */
import type { CharacterEmbedding } from '@/types/character';

export interface LocalizedCharFields {
    name: string;
    anime: string;
    tags: string[];
    personality: string[];
...
export function getLocalizedChar(char: CharacterEmbedding, lang: string): LocalizedCharFields {
    ...
    return {
        ...
        quote: isJa && char.heroine_quote_ja ? char.heroine_quote_ja :
            isZh && char.heroine_quote_zh_tw ? char.heroine_quote_zh_tw :
                isKo ? char.heroine_quote : char.heroine_quote_en,
    };
}
